          ...
          <div className="flex flex-col gap-1 mt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {project.tech.map((t, i) => (
                <span
                  key={i}
                  className="text-xs font-mono px-2 py-1 rounded-full bg-cyan-900/70 text-cyan-200 border border-cyan-800/30 backdrop-blur-sm"
                >
                  {t}
                </span>
              ))}
            ...
            {/* Processor badges */}
            {project.processor?.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {project.processor.map((cpu, i) => (
                  <span
                    key={i}
                    className="text-xs font-mono px-2 py-1 rounded-full bg-purple-800/50 text-purple-200 border border-purple-500/30 backdrop-blur-sm"
                  >
                    {cpu}
                  </span>
                ))}
